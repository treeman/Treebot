...
class Insults extends DefaultPlugin
{
    ...
    override process_privmsg ($sender, $target, $msg)
    {
        ...
        if (defined ($retort = $monkey_island1{$msg})) {
            if ($shall_succeed) {
                Irc::send_privmsg ($target, $retort);
            }
            else {
                $retort = $failed_retorts[rand @failed_retorts];
                Irc::send_privmsg ($target, $retort);
            }
        }
        elsif (defined ($retort = $sword_master{$msg})) {
            if ($shall_succeed) {
                Irc::send_privmsg ($target, $retort);
            }
            else {
        ...
        elsif (defined ($retort = $monkey_island3{$msg})) {
            if ($shall_succeed) {
                Irc::send_privmsg ($target, $retort);
            }
            else {
        ...
        elsif (defined ($retort = $captain_rottingham{$msg})) {
            if ($shall_succeed) {
                Irc::send_privmsg ($target, $retort);
            }
            else {
        ...
        elsif (defined ($retort = $monkey_island4{$msg})) {
            if ($shall_succeed) {
                Irc::send_privmsg ($target, $retort);
            }
            else {
